...
// ── Helper: log in and wait for app shell ──────────────────────────────────
async function loginAs(page: Parameters<typeof test>[1] extends { page: infer P } ? never : any, email: string, password: string) {
...
test.describe('Subscription Page', () => {
  ...
  test('owner can navigate to subscription page', async ({ page }) => {
    ...
    await expect(page.getByText(/方案|plan|subscription/i)).toBeVisible({ timeout: 10000 })
  })
